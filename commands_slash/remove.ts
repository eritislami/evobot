...
export default {
  data: new SlashCommandBuilder()
    ...
    .addStringOption((option) => option.setName("songslot").setDescription(i18n.__("remove.description")).setRequired(true)),
  execute(interaction: CommandInteraction) {
    const guildMemer = interaction.guild!.members.cache.get(interaction.user.id);
    // @ts-ignore
    const removeArgs = interaction.options.getString("songslot")
