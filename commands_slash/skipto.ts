...
export default {
  data: new SlashCommandBuilder()
    ...
    .addIntegerOption((option) => option.setName("number").setDescription(i18n.__("skipto.args.number")).setRequired(true)),
  execute(interaction: CommandInteraction) {
    ...
    if (!playlistSlotArg || isNaN(playlistSlotArg))
      return interaction
        .reply({content: i18n.__mf("skipto.usageReply", { prefix: bot.prefix, name: module.exports.name }), ephemeral: true})
        .catch(console.error);
    ...
    interaction.reply({content: i18n.__mf("skipto.result", { author: interaction.user.id, arg: playlistSlotArg - 1 })})
      .catch(console.error);
  }
};
