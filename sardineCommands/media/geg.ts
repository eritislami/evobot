...
command.data = new SlashCommandBuilder().setName('geg').setDescription('Good Evening Gaming');
command.execute = async (interaction: ChatInputCommandInteraction) => {
	return interaction.reply('Good Evening Gaming').then(async (value: InteractionResponse) => {
		if (value.interaction.channel instanceof TextChannel) {
			await value.interaction.channel.send(getGif('geg'));
		}
	});
};
