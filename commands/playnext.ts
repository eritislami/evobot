...
export default {
  data: new SlashCommandBuilder()
    ...
    .addStringOption((option) => option.setName("song").setDescription("The song you want to play next").setRequired(true)),
  cooldown: 3,
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    if (queue) {
      queue.songs.splice(1, 0, song); // Fügt das Lied an die zweite Position in der Queue ein
    ...
    newQueue.songs.splice(1, 0, song); // Fügt das Lied an die zweite Position in der Queue ein
    newQueue.play();
