...
command.data = new SlashCommandBuilder().setName('brbg').setDescription('Be right back gaming');
command.execute = async (interaction: ChatInputCommandInteraction) => {
	return interaction.reply('Be right back gaming').then(async (value: InteractionResponse) => {
		if (value.interaction.channel instanceof TextChannel) {
			await value.interaction.channel.send(getGif('brbg'));
		}
	});
};
